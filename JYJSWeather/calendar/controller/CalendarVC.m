...
@interface CalendarVC ()<calendarBtnViewDelegate,lisuanCalendarViewDelegate,pickerViewDelegate>
{
    FMDatabase * _db;
...
//    leftBarButton.imageView.frame = CGRectMake(0, 0, leftBarButton.frame.size.width/2, leftBarButton.frame.size.height/2);
    ...
    UIBarButtonItem *backButton = [[UIBarButtonItem alloc]initWithTitle:@"返回" style:UIBarButtonItemStyleDone target:self action:@selector(goBackLastController)];
        UIBarButtonItem *todayButton = [[UIBarButtonItem alloc]initWithTitle:@"今天" style:UIBarButtonItemStyleDone target:self action:@selector(backTodayAction)];
    self.navigationItem.leftBarButtonItems = [NSArray arrayWithObjects:backButton,todayButton, nil];
...
//    _calendarView.backgroundColor = UIColorFromRGB(0xfafafa);// 背景颜色
    ...
    /**
     创建一个视图作为背景
     */
    _today = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 14.5 * CALENDARBTNPADDING )];
    _today.backgroundColor = [UIColor colorWithPatternImage:image];
